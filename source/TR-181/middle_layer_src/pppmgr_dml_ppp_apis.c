...
bool PppMgr_EnableIf (UINT InstanceNumber, bool Enable)
{
    ...
    if (Enable == true)
    {
        ...
        {
            CcspTraceError(("%s %d: posting PPPMGR_EXEC_PPP_CLIENT event to Q failed for instance %d\n", __FUNCTION__, __LINE__, InstanceNumber));
            return false;
        }
    }
    else
    {
        ...
        {
            CcspTraceError(("%s %d: failed to stop ppp client on instance %d\n", __FUNCTION__, __LINE__, InstanceNumber));
            return false;
        }
    }
    return true;
...
#ifndef DYNAMIC_CONFIGURE_PPP_LOWERLAYER
    /* Get LowerLayer */
